...
public record ModMetadata : AbstractModMetadata
{
    ...
    public override SemanticVersioning.Version Version { get; } = new("2.0.0");
    ...
    : IOnLoad
{
    ...
    public Task OnLoad()
    {
        string configDirectory =
            Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)!, "db", "items");
        commonLib.ItemService.CreateCustomItems(configDirectory);

        // A path to the mods files we use below
        var pathToMod = modHelper.GetAbsolutePathToModFolder(Assembly.GetExecutingAssembly());

        var quest = modHelper.GetJsonDataFromFile<Quest>(pathToMod, "db/quests/quest.json");

        databaseServer.GetTables().Templates.Quests[quest.Id] = quest;
